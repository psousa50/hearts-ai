...
from hearts_game_core.strategies import Strategy, StrategyGameState, Player
...
class MonteCarloStrategy(Strategy):
    def __init__(self, num_simulations: int = 5000, exploration_weight: float = 1.4, random_manager: RandomManager = None):
    ...
    def choose_card(self, strategy_game_state: StrategyGameState) -> Card:
        ...
        all_cards_in_players_hands = [card for card in self.all_cards if card not in all_played_cards and card not in strategy_game_state.player_hand]
    ...
    def _monte_carlo_tree_search(self, strategy_game_state: StrategyGameState, 
                                valid_moves: List[Card], 
                                all_cards_in_players_hands: List[Card], 
                                num_simulations: int) -> Card:
    ...
    def _create_game_for_simulation(self, strategy_game_state: StrategyGameState, all_cards_in_players_hands: List[Card]) -> HeartsGame:
        """Create a game for simulation."""
    ...
    def _get_cache_key(self, strategy_game_state):
        ...
        return (current_trick, player_hand, prev_tricks, strategy_game_state.player_score)
