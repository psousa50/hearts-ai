...
class SimulationStrategy(Strategy):
    ...
    def choose_card(self, strategy_game_state: StrategyGameState) -> Card:
        ...
        for moves in self.batch_moves(grouped_valid_moves, self.number_of_processes):
            futures = [self.executor.submit(run_simulations_for_move, strategy_game_state, move, simulations_per_move, all_cards_in_players_hands, self.deck, self.random_manager) for move in moves]
...
def create_game_for_simulation(strategy_game_state, first_player_index, current_player_idx, all_cards_in_players_hands, deck, random_manager: RandomManager):
...
def run_simulations_for_move(strategy_game_state, move, simulations_per_move, all_cards_in_players_hands, deck, random_manager: RandomManager):
